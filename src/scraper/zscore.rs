...
pub async fn busca_zscore(
	...
	let hist1 = conn.get_quote_history(&ticker_a, start, end).await?;
	let hist2 = conn.get_quote_history(&ticker_b, start, end).await?;

	let quotes1 = hist1.quotes()?;
	let quotes2 = hist2.quotes()?;
	...
	for (q1, q2) in quotes1.iter().zip(quotes2.iter()) {
		...
		let desvio = (spreads.iter().map(|x| (x - media).powi(2)).sum::<f64>() / spreads.len() as f64).sqrt();
		let zscore = if desvio != 0.0 { (spread - media) / desvio } else { 0.0 };

		let data = OffsetDateTime::from_unix_timestamp(q1.timestamp)?;
		linhas.push(format!("{},{:.2},{:.2},{:.2},{:.2},{:.2},{:.2}",
			data.date(), q1.close, q2.close, spread, media, desvio, zscore));
	}
